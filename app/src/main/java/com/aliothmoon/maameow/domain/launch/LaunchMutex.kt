 ...
 * 进程内内存；不跨进程；start 成功/失败后应立即 [release]
 */
class LaunchMutex {
     ...
     * 是否已被**其他**请求占用。
     *
     * 与 [tryAcquire] 的区别：本方法只做判断、不改状态，
     * 用于在抢占前决定"要不要先取消上一轮"，避免误伤自己。
     */
    @Synchronized
    fun isOccupiedByOther(requestId: String): Boolean {
        val existing = holder ?: return false
